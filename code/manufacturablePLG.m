classdef manufacturablePLG < PLG
    % manufacturablePLG enables the custom colouring of a lattice file 
    ...
    methods
        function obj = manufacturablePLG(file)
            obj = obj@PLG(file);
        ...
        function obj = runManufacturability(obj,file)
            ...
            % get a list of colours
             ...
             obj.colour = manufacturablePLG.letter2numColour(obj.colour); % change letter colours to array colours
        end
        function [f,a] = plot(obj)
            [f,a] = plot@PLG(obj,obj.colour);
        end
    end
    methods (Access = protected) % sub functions
        ...
        function obj = readProcessMap(obj,file)
            ...
            obj.processMap = manufacturablePLG.readTables(t{1}(2:end),numTables,rows,cols);
        end
    ...
    methods (Static)
        function d = readTables(data,numTables,rows,cols)
            ...
            for inc = 1:numTables
                ...
                if inc==1
                    ...
                    if test1 || test2
                        error('manufacturablePLG:inconsistentTables','Input process map tables are inconsistent');
                    end
                end
            end
        end
        function colourNum = letter2numColour(colourLet)
            ...
            colourNumF = manufacturablePLG.letter2numColour(colourLet(2:end));
            colourNum = [colourNumC;colourNumF];
        end
    end
    methods (Access = ?TestManufacturable)
